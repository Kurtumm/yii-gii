...
class CrudCode extends CCodeModel
{
	...
	public function generateActiveLabel($modelClass,$column, $class='')
	{
		return "\$form->labelEx(\$model,'{$column->name}', array('class'=>'{$class}'))";
	}

	public function generateActiveField($modelClass,$column, $class='')
	{
		if($column->type==='boolean')
			return "\$form->checkBox(\$model,'{$column->name}')";
		elseif(stripos($column->dbType,'text')!==false)
			return "\$form->textArea(\$model,'{$column->name}',array('rows'=>6, 'cols'=>50, 'class'=>'{$class}'))";
		else
		{
			if(preg_match('/^(password|pass|passwd|passcode)$/i',$column->name))
